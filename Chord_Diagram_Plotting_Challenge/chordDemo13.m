% chordDemo13

% @author : slandarer
% 公众号  : slandarer随笔
% 知乎    : slandarer
clc;clear
rng(5)

dataMat = randi([1,40], [7,4]);
dataMat(rand([7,4]) < .3) = 0;
colName = compose('MATLAB%d', 1:4);
rowName = compose('SL%d', 1:7);

figure('Units','normalized', 'Position',[.02,.05,.7,.85])
CC = chordChart(dataMat, 'RowName',rowName, 'ColName',colName, 'Sep',1/80, 'LRadius',1.32, 'TickMode','auto', 'Arrow','on');
CC = CC.draw();
CC.tickState('on')
CC.tickLabelState('on')
CC.setFont('FontSize',17, 'FontName','Cambria')
CC.setTickFont('FontSize',10, 'FontName','Cambria')
% Modify the color of the blocks above (修改上方方块颜色)
CListT = [0.49,0.64,0.53; 0.75,0.39,0.35; 0.80,0.74,0.42; 0.40,0.55,0.66];
CC.setSquareColorT(CListT)
% Modify the color of the blocks below (修改下方方块颜色)
CListS = [0.91,0.91,0.97; 0.62,0.95,0.66; 0.91,0.61,0.20; 0.54,0.45,0.82;
          0.99,0.76,0.81; 0.91,0.85,0.83; 0.53,0.42,0.43];
CC.setSquareColorS(CListS)
% Modify chord color (修改弦颜色)
CC.setChordColorBySquareS()
% Set line color and line width (设置线条颜色及粗细)
CC.setSquareT('EdgeColor','k', 'LineWidth',1)
CC.setSquareS('EdgeColor','k', 'LineWidth',1)
CC.setChord('EdgeColor','k', 'LineWidth',1, 'FaceAlpha',.5)
CC.setTick('LineWidth',1)
% Draw legend (绘制图例)
lgdHdl = legend(CC.squareSHdl, rowName, 'Location','eastoutside', ...
    'FontSize',16, 'FontName','Cambria', 'Box','off');
lgdHdl.ItemTokenSize = [18,8];